...
def mecanico(request):
    ...
    if request.method =="POST":
        ...
        if valor_buscado != "":
            mecanico = Mecanico.objects.filter(categoria__nombre = valor_buscado, rut =valor_buscado)
...
def registro(request):
    ...
    if request.method == "POST":
        ...
        grupoMec = Group.objects.get(name=tipo)
        ...
        try:
            ...
            user = authenticate(username=usu.name, password=password)
            login(request, user)
